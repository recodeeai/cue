---
title: "Hizir-SamuelP/deep-trip-planning — Claude Code skill discovered by cue"
description: "A Claude Skill that turns a list of places into an itinerary you can actually follow."
layout: page
updated: 2026-09-04
tags: [claude-code, skill, core]
---

# 💎 [Hizir-SamuelP/deep-trip-planning](https://github.com/Hizir-SamuelP/deep-trip-planning)

**★ 1** · exceptional (score 8.8) · Python · agent-skills, ai-agent, claude-skill, itinerary, travel-planning

> A Claude Skill that turns a list of places into an itinerary you can actually follow.

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 8.8 based on:
- ⭐ 1 stars
- 🗓️ Last pushed: 2026-09-04
- 🏷️ Tags: agent-skills, ai-agent, claude-skill, itinerary, travel-planning

## Best fit cue profiles

[core](../core.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add Hizir-SamuelP/deep-trip-planning --profile core
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/Hizir-SamuelP/deep-trip-planning)
