---
title: "greenstevester/db-architecture-reviewer — Claude Code skill discovered by cue"
description: "Claude Code skill that documents a PostgreSQL schema and reviews its design in one run"
layout: page
updated: 2026-09-05
tags: [claude-code, skill, backend]
---

# 💎 [greenstevester/db-architecture-reviewer](https://github.com/greenstevester/db-architecture-reviewer)

**★ 0** · exceptional (score 8) · HTML · claude-code, claude-skill, code-review, data-modeling, database

> Claude Code skill that documents a PostgreSQL schema and reviews its design in one run

## Why cue indexed it

cue ran [GitHub Code Search](https://docs.github.com/en/search-github/searching-on-github/searching-code) for `filename:SKILL.md` and found this repo. It scored 8 based on:
- ⭐ 0 stars
- 🗓️ Last pushed: 2026-09-05
- 🏷️ Tags: claude-code, claude-skill, code-review, data-modeling, database, documentation, postgresql, schema

## Best fit cue profiles

[backend](../backend.md)



## Install via cue

```bash
npm install -g cue-ai
cue skills add greenstevester/db-architecture-reviewer --profile backend
```

## About

This page was auto-generated by [cue](https://github.com/opencue/cuecards) — an open-source agent profile manager for Claude Code, Codex, Cursor, Cline, Gemini, Copilot, and 4 other AI coding agents. cue scopes skills + MCPs + plugins per-directory so each project only loads what it needs.

**Repo author:** if you'd rather we don't list this skill, add `<!-- cue: ignore -->` to your README and we'll skip it permanently.

---

[← back to all profiles](../index.md) · [view repo on GitHub →](https://github.com/greenstevester/db-architecture-reviewer)
